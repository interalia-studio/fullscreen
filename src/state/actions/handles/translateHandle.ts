import type { Action, CustomBinding } from "~/src/state/constants";
import { getPagePoint } from "~/src/state/helpers";
import { getShapeUtils, Shape } from "~/src/shapes";
...
export const translateHandle: Action = (data, payload: TLPointerInfo) => {
  ...
  data.pageState.selectedIds.forEach((id) => {
    ...
    if (payload.shiftKey) {
      const A =
        initialShape.handles[pointedHandleId === "start" ? "end" : "start"]
          .point;
    ...
    handlePoints[pointedHandleId] = Vec.add(
      handlePoints[pointedHandleId],
      delta
    );
    ...
    const oppositeBindingTargetId = Object.values(data.page.bindings).find(
      (binding) =>
        binding.fromId === shape.id && binding.handleId === oppositeHandleId
    )?.toId;
      ...
      Object.values(data.page.shapes)
        .filter(
          (shape) =>
            ...
            shape.id !== oppositeBindingTargetId
        )
        .forEach((potentialTarget) => {
          ...
          if (Utils.pointInBounds(handlePoint, bounds)) {
            const dist = Vec.dist(
              handlePoint,
              utils.getCenter(potentialTarget)
            );
            if (dist < minDistance) {
              minDistance = dist;
    ...
    const oldBinding = Object.values(data.page.bindings).find(
      (binding) =>
        binding.fromId === shape.id && binding.handleId === pointedHandleId
    );
    ...
    const offset = Utils.getCommonTopLeft([
      handlePoints.start,
      handlePoints.end,
    ]);
